...
- **手臂是独立盒体，漏了它村民就没有手。** `VillagerModel` 里手臂是 `texOffs(44,22)` 的 4×8×4 盒体，
  正面片 `(44+4, 22+4) = (48,26)` 尺寸 4×8，而且**两条手臂共用同一个 texOffs**（模型里是两个盒体、
  贴图偏移相同），所以左右两处同源。判定它属于哪块区域的办法：数皮肤上的像素范围——顶面+底面是一道
  8×4 的横带（`cols 48–55 / rows 22–25`），四个侧面是 16×8 的一片（`cols 44–59 / rows 26–33`），
  宽度正好 `2×(dw+dz)=16`，与 4×8×4 唯一吻合。
- **竖直比例不能靠重叠硬塞。** 模型竖直方向是头 `-10…0`、身 `0…12`、腿 `12…24`，总高 34 单位；
  村民模型整体缩放 0.9375，所以 34 单位 ≈ 31.9 像素，正好是一条 32 行画布。把「头 10 + 袍 12 + 腿 12」
  用 2 行重叠压进 32 行是**错的**（实测症状：袍和腿各长约 1 行、腿整体上移 2 行，"各部位比例不对"）。
  正确做法是把 34→32 的压缩按部位摊掉：**头 0–9 行、袍 10–20 行、腿 21–31 行**，每块 1:1 取面片、
  不重采样也不重叠；袍让掉的是被头压住的领口接缝，腿让掉的是被袍子压住的顶端，底部腰带/鞋面都保留。
  手臂按模型 `y=1…9` 挂在肩线（袍子顶行之下 1 行）起 8 行，鼻子按模型 `y=-6…-2` 落在第 4–7 行。
