...
/**
 * MC 内置兜底贴图：16×16 字符画像素 SVG，编译成 data URI 内联进 bundle。
 *
 * 首选是在线原版材质（见 ONLINE_TEX），但 webview 可能离线或加载失败，
 * 所以每张图都有这份零网络依赖的像素画兜底，永远不会裂图。
 *
...
/**
 * 原版 MC 贴图的在线地址（jsDelivr 镜像的 InventivetalentDev/minecraft-assets，
 * 已逐一验证可达）。
 *
 * 两个例外：
 * - 树叶不用在线图——原版 oak_leaves.png 是去色素材，绿色靠游戏内生物群系
 *   染色，直接显示是灰的；TEX.leaves 已是还原染色后的原版树叶 PNG。
 * - 村民用 Minecraft Wiki 的正面渲染图 Plains_Villager_Base_JE1_BE1.png
 *   （文件名经 wiki API 核实存在）；wiki 图床有防盗链，加载失败自动回退
 *   到内置的 16×32 像素村民。
 *
 * 加载策略：组件挂载后用 new Image() 预载，加载成功的换成在线贴图，
 * 失败/离线时继续用内置像素画兜底——不会出现裂图。
 */
const MC_CDN = 'https://cdn.jsdelivr.net/gh/InventivetalentDev/minecraft-assets@1.20.4/assets/minecraft/textures/'
const ONLINE_TEX = {
  grass: MC_CDN + 'block/grass_block_side.png',
  dirt: MC_CDN + 'block/dirt.png',
  goldOre: MC_CDN + 'block/gold_ore.png',
  log: MC_CDN + 'block/oak_log.png',
  table: MC_CDN + 'block/crafting_table_front.png',
  apple: MC_CDN + 'item/apple.png',
  bread: MC_CDN + 'item/bread.png',
  goldenApple: MC_CDN + 'item/golden_apple.png',
  goldIngot: MC_CDN + 'item/gold_ingot.png',
  villager: 'https://minecraft.wiki/images/Plains_Villager_Base_JE1_BE1.png',
}

/** 预载在线原版贴图；返回 { ...TEX兜底, 加载成功的换在线地址 }。 */
function useOnlineTextures() {
  const [tex, setTex] = React.useState(TEX)
  React.useEffect(() => {
    if (typeof globalThis.Image !== 'function') return undefined
    let alive = true
    const next = { ...TEX }
    for (const key of Object.keys(ONLINE_TEX)) {
      const img = new globalThis.Image()
      img.onload = () => {
        if (!alive) return
        next[key] = ONLINE_TEX[key]
        setTex({ ...next })
      }
      img.src = ONLINE_TEX[key]
    }
    return () => {
      alive = false
    }
 ...
 * 贴图优先用在线原版 MC 材质（jsDelivr），加载失败自动回退内置像素画。
...
function McGame(props) {
  ...
  // 在线原版贴图加载成功的会覆盖内置像素兜底；tex 键与 TEX 一致。
  const tex = useOnlineTextures()
...
function SurvivalPanel(props) {
  ...
  // 饱食度充足时禁用（≥2 步余量），防误喂；金苹果始终可点，它是应急与复活选项。
  // 背包没有存货的食物一律禁用——喂食从背包消耗，没货喂不了。
      ...
      React.createElement('div', { className: 'dsv-actions' },
        props.foods.map((food, index) => {
          ...
          return React.createElement('button', {
            ...
            disabled: busy || count <= 0 || (index !== props.foods.length - 1 && enough),
            title: count <= 0 ? '背包里没有' + food.label + '，去 MC 小游戏里采集或合成' : '喂食 1 个' + food.label,
