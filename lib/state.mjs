...
export function createSurvivalState(options = {}) {
  ...
  return {
     ...
     *
     * 两条门槛（改动前先读，实测踩过）：
     * 1. **饿死时只有金苹果能复活**。早期版本让任意食物都带复活加成，结果面包成了
     *    比金苹果更好用的复活道具（面包 +45 也照样解除饿死），金苹果的合成价值归零；
     *    现在死后喂苹果/面包直接被拒，复活必须付金苹果的代价。
     * 2. **满饱食度不吃**。面板会禁用，但工具入口（模型可调用的 survival_feed）
     *    绕得过去，所以规则落在状态机里：饱食度和生命都满时不消耗食物。
     *
     * @param food 食物键（apple / bread / golden_apple）。
     * @returns 结果对象；未知食物、背包没有存货、饿死时喂普通食物、满饱食度时喂食
     *   都返回 ok: false。
     */
    feed(food) {
      ...
      if (state.dead && food !== REVIVE_FOOD) {
        return {
          ok: false,
          message: '你已经饿死了，' + spec.label + '救不回来：只有'
            + FOODS[REVIVE_FOOD].label + '能复活。',
        }
      }
      if (inventory[food] <= 0) {
        return {
      ...
      if (state.hunger >= config.maxHunger && state.health >= config.maxHealth) {
        return {
          ok: false,
          message: '饱食度和生命都是满的，先别浪费' + spec.label + '：干活掉饿了再喂。',
        }
      }
      inventory[food] -= 1
    ...
    snapshot() {
      ...
      return {
        ...
        reviveHp: REVIVE.hp,
        // 唯一复活食物的键：客户端据此禁用死后无效的按钮，不靠食物表的位置猜。
        reviveFood: REVIVE_FOOD,
