...
export function apply(ctx) {
  ...
  ctx.on('tools/pre-execute', async (exec, next) => {
    ...
    if (exec.name === 'survival_feed') return next()
    return {
      kind: 'deny',
  ...
  ctx.tools.register(defineTool({
    ...
    description: '生存模式（全局共享）：进食恢复饱食度，影响整个 DSH 的所有会话。'
      + '可选：apple=苹果(+25 饱食)，steak=牛扒(+55 饱食/+2 生命)，'
      + 'golden_carrot=金胡萝卜(+100 饱食/+4 生命)。'
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          hunger: { type: 'integer', required: true },
